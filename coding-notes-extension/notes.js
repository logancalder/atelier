...
function childMarkup(item) {
  return `<article class="child-record"><div><a href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.title)}</a><span>${site(item)} · ${formatTime(item.seconds)} · ${submissionCount(item)} submissions</span></div><div class="child-actions"><div class="badges">${badges(item)}</div>${site(item) === "neetcode" ? `<button data-unlink-key="${escapeHtml(item.key)}" type="button">Unlink</button>` : ""}</div>${item.notes ? `<p>${escapeHtml(item.notes)}</p>` : ""}</article>`;
}

function cardMarkup({ parent, children }) {
  return `<article class="card"><div class="card-head"><div><h2><a href="${escapeHtml(parent.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(parent.title)}</a></h2><div class="site">${site(parent)} data${parent.titleSource ? ` · ${parent.titleSource} title` : ""} · ${escapeHtml(category(parent))} · ${formatTime(parent.seconds)} · ${submissionCount(parent)} submissions</div></div><div class="card-tools"><div class="badges">${badges(parent)}</div>${editButton(parent)}</div></div><div class="notes ${parent.notes ? "" : "muted"}">${parent.notes ? escapeHtml(parent.notes) : "No notes yet."}</div>${unlinkMarkup(parent)}${parent.titleSource ? "" : manualLinkMarkup(parent)}${children.length ? `<details><summary>Show linked ${children.map(site).join(", ")} data</summary><div class="children">${children.map(childMarkup).join("")}</div></details>` : ""}</article>`;
}

function listRow({ parent }) {
  return `<div class="problem-row"><div class="row-main"><a class="row-title" href="${escapeHtml(parent.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(parent.title)}</a><span>${site(parent)} · ${formatTime(parent.seconds)} · ${submissionCount(parent)} subs</span></div><div class="badges compact">${compactBadges(parent)}</div>${editButton(parent)}</div>`;
}
...
function renderRetries() {
  ...
  document.getElementById("retry-list").innerHTML = ranked.length ? ranked.map(({ parent, reasons }, index) => `<a class="retry-row" href="${escapeHtml(parent.url)}" target="_blank" rel="noopener noreferrer"><span class="retry-rank">${index + 1}</span><span><b>${escapeHtml(parent.title)}</b><small>${escapeHtml(reasons.slice(0, 2).join(" · "))}${reasons.length > 2 ? ` · +${reasons.length - 2}` : ""}</small></span><i>↗</i></a>`).join("") : '<p class="retry-empty">Nothing is flagged for a retry yet.</p>';
}
...
async function init() {
  ...
  document.querySelector(".view-switch").addEventListener("click", (event) => { const button = event.target.closest("[data-view]"); if (!button) return; currentView = button.dataset.view; document.querySelectorAll("[data-view]").forEach((item) => { item.classList.toggle("active", item === button); item.setAttribute("aria-pressed", String(item === button)); }); render(); });
