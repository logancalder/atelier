...
async function init() {
  ...
  document.getElementById("summary").innerHTML = `<div class="metric"><b>${groups.length}</b><span>Problems</span></div><div class="metric"><b>${attempts}</b><span>Submissions</span></div><div class="metric"><b>${holes}</b><span>Hole in one${holes === 1 ? "" : "s"}</span></div>`;
  document.getElementById("empty").hidden = groups.length > 0;
  document.getElementById("problems").innerHTML = groups.slice(0, 12).map(({ parent, children }) => { const count = submissionCount(parent); return `<article class="problem"><div class="problem-top"><a href="${escapeHtml(parent.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(parent.title)}</a><time>${formatTime(parent.seconds)}</time></div><div class="problem-meta"><span>${count} submission${count === 1 ? "" : "s"}${children.length ? ` · +${children.length} linked` : ""}</span>${parent.holeInOne ? '<p class="hole">★ Hole in one</p>' : ""}</div><p class="problem-preview">${escapeHtml(parent.notes || "No notes yet.")}</p></article>`; }).join("");
