...
export default async function SessionsPage({
  ...
  return (
    ...
    >
      <TutoringTabs active="calendar" />
      <div className="calendar-toolbar">
        <Link href={`/studio?view=calendar&week=${prev}`} className="text-sm text-mute hover:text-ink">
          ← Previous
        </Link>
        <p className="text-center font-serif text-base sm:text-xl">
          Week of {formatShortDate(toDateKey(weekStart))}
        </p>
        <Link href={`/studio?view=calendar&week=${next}`} className="text-sm text-mute hover:text-ink">
          Next →
        </Link>
      </div>
